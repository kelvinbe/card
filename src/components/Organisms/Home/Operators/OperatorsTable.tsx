...
import { transformMerchantsData } from '@/utils/utils'
...
function OperatorsTable<TData, TValue>({columns, data}:DataTableProps<TData, TValue>) {
  ...
  // const operatorsData = transformMerchantsData(allOperators)

  console.log('operatorsData', allOperators)
  ...
  return (
    <Box>
          <OperationalHeader filterData={filterData}  buttonIcon={<Plus className="mr-2 h-4 w-4" />} buttonFunction={() => navigate('/createOperators')}  search filter buttonTitle='Create Operator' searchTerm={globalFilter} onSearch={setGlobalFilter}   table={table} name='Operators' />  
        {selectedOperation &&  <Box m={5} display={'flex'} justifyContent={'space-between'} alignItems={'center'}>
          <Box display={'flex'}>
          <FilterByComponent columnValues='merchant' clear={columnValue} table={table} variant='merchant' borderRadius='full'  buttonWidth='150' chevronColor='#000' textColor='#fff'  popOverWidth='100' /> 
