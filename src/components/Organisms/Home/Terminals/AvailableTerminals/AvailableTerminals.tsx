...
function AvailableTerminalsTable<TData, TValue>({columns, data}:DataTableProps<TData, TValue>) {
  ...
  return (
    <Box>
          <OperationalHeader buttonFunction={() => navigate('/home/createTerminal')} dropDown search filter buttonTitle='Create Terminal' buttonIcon={<Plus className="mr-2 h-4 w-4" />} searchTerm={globalFilter} onSearch={setGlobalFilter}   table={table} name='Terminals' />  
        {selectedOperation &&  <Box m={5} display={'flex'} justifyContent={'space-between'} alignItems={'center'}>
          <Box display={'flex'}>
          <FilterByComponent table={table} variant='location' borderRadius='full'  buttonWidth='150' chevronColor='#000' textColor='#fff' data={locationData} popOverWidth='100' /> 
         {selectedOperation === 'Status' && <FilterByComponent table={table} variant='status' borderRadius='full'  buttonWidth='150' chevronColor='#000' textColor='#fff' data={StatusData} popOverWidth='100' /> }
         </Box>
         <ButtonComponent onClick={() => setSelectedOperation(null)} icon={<X size={15} className="mt-1" color="#000" />} textColor='#000' title='Clear Filter' bgColor='transparent' />
...
export default AvailableTerminalsTable